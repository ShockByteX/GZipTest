...
namespace GZipTest.Compression.GZip
{
    ...
    {
        private byte[] _headerBytes = new byte[] { 0x1f, 0x8b, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00 }; // ID1, ID2, CM, FLG, MTIME
        ...
        protected override void ProcessThreadFunction()
        {
            ...
            while (!_isCanceled && _readQueue.TryDequeue(out block)) _writeQueue.Enqueue(Decompress(block));
        }
        protected override void WriteThreadFunction()
        {
            ...
            {
                CompressionBlock block;
                while (!_isCanceled && _writeQueue.TryDequeue(out block)) stream.Write(block.Data, 0, block.Data.Length);
            }
        }
        private int GetNextBlockLength(BinaryReader reader)
        {
            ...
            if (!reader.ReadBytes(_headerBytes.Length).SequenceEqual(_headerBytes)) throw new InvalidDataException("Source file data is not correct.");
            ...
            int offset = _largeBuffer.Find(_headerBytes, 0, length);
            stream.Position = lastPosition;
            if (offset == -1) return (int)(stream.Length - stream.Position);
            return offset + _headerBytes.Length;
        }
